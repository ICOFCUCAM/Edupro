...
const COUNTRY_FORMATS: Record<string, any> = {
  'Nigeria': {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const columns = countryFormat.columns;
    const curriculum = countryFormat.curriculum;
...
Generate a comprehensive lesson note that follows the ${country} education system format. The lesson note must include content for ALL of these columns: ${columns.join(', ')}.

IMPORTANT: Generate content for 5 days/periods of the week. Each day should have detailed, actionable content.
...
Make the content educationally sound, age-appropriate, and aligned with ${country}'s national curriculum standards. Be specific and detailed in each cell.`;
    ...
    try {
      ...
      lessonContent = JSON.parse(jsonStr);
    } catch (e) {
      lessonContent = {
        title: `${subject} - ${topic}`,
        metadata: { subject, topic, level, country, curriculum, week: week || 1 },
        columns: columns,
        rows: [{ [columns[0]]: week || 1 }],
        rawContent: data.choices?.[0]?.message?.content || 'Generation failed',
        teacherNotes: '',
    ...
    return new Response(JSON.stringify({ success: true, lessonNote: lessonContent, countryFormat }), {
